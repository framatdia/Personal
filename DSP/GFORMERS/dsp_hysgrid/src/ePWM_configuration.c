...
{
	// Phase c
	EPwm1Regs.TBCTL.bit.HSPCLKDIV = TB_DIV1; 		// TBCLK = SYSCLKOUT (for 20kHz with 50us PWM_TIMER_TBPRD)
    #if (DEVICE==0)
	EPwm1Regs.TBCTL.bit.CLKDIV = TB_DIV4;           //TBCLK*4 = SYSCLKOUT (for 5kHz)
    #else
    EPwm1Regs.TBCTL.bit.CLKDIV = TB_DIV2;           //TBCLK*2 = SYSCLKOUT (for 10kHz)
    #endif
	...
	EPwm5Regs.TBPRD = PWM_TIMER_TBPRD; 				// f = 20 kHz
