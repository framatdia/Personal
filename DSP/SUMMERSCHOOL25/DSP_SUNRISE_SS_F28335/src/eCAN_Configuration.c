...
struct references eCAN_RW(struct ADC_conv ADC_processed, struct rms rms_calc, struct powers p, struct PLL_output PLL_Park, struct state_m state)

{
	...
	read.start_FSM=read_register & 0x1;
	read.start_AC=(read_register & 0x2)>>1;
	/*read.start_DC=(read_register & 0x4)>>2;
	read.sync=(read_register & 0x8)>>3;
	read.enable_P_ref=(read_register & 0x10)>>4;
	read.enable_HFPS=(read_register & 0x20)>>5;
	// read.enable_resonant=(read_register & 0x40)>>6;
	read.start_Primary=(read_register & 0x80)>>7;*/

	read_register=mailbox_read(17);

	read.etas=((signed int)(read_register & 0xFFFF))*0.001;
	//read.eta_dc=((signed int)(read_register>>16 & 0xFFFF))*0.001;
	read.id_ref=((signed int)(read_register>>32 & 0xFFFF))*0.1;
	read.iq_ref=((signed int)(read_register>>48 & 0xFFFF))*0.1;

	//read_register=mailbox_read(18);
	/*read.V_h_min=((signed int)(read_register & 0xFFFF));
	read.V_pk_min=((signed int)(read_register>>16 & 0xFFFF));
	read.I_pk_max=((signed int)(read_register>>32 & 0xFFFF));
	read.I_l_max=((signed int)(read_register>>48 & 0xFFFF));*/

	read_register=mailbox_read(19);
	read.p_ref=((signed int)(read_register & 0xFFFF));
	read.q_ref=((signed int)(read_register>>16 & 0xFFFF));
	//read.H=((signed int)(read_register>>32 & 0xFFFF));

	read_register=mailbox_read(20);

	read.vdc_ref=(signed int)(read_register & 0xFFFF);
	//read.V_uc_ref=(signed int)(read_register>>16 & 0xFFFF);
	//read.Pa_ref= ((signed int)(read_register>>32 & 0xFFFF));
	//read.Qa_ref= ((signed int)(read_register>>48 & 0xFFFF));

    //read_register=mailbox_read(21);

   /* read.Pb_ref=(signed int)(read_register & 0xFFFF);
    read.Qb_ref=(signed int)(read_register>>16 & 0xFFFF);
    read.Pc_ref= ((signed int)(read_register>>32 & 0xFFFF));
    read.Qc_ref= ((signed int)(read_register>>48 & 0xFFFF));*/
